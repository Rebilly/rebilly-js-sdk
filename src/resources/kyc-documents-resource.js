/**
* This file was auto-generated by rebilly-sdk-generator.
* Do not make direct changes to this file.
*/

export default function KycDocumentsResource({apiHandler}) {
  return {
    /**
     * @param { rebilly.GetKycDocumentCollectionRequest } request
     * @returns { rebilly.GetKycDocumentCollectionResponse } response
     */
    getAll({limit = null, offset = null, filter = null, sort = null} = {}) {
      const params = {limit, offset, filter, sort};
      return apiHandler.getAll(`kyc-documents`, params);
    },
    /**
     * @param { rebilly.CreateKycDocumentRequest } request
     * @returns { rebilly.PostKycDocumentResponse } response
     */
    create({id = '', data}) {
      return apiHandler.create(`kyc-documents/${id}`, id, data);
    },
    /**
     * @returns { rebilly.GetKycDocumentResponse } response
     */
    get({id}) {
      return apiHandler.get(`kyc-documents/${id}`);
    },
    /**
     * @returns { rebilly.PutKycDocumentResponse } response
     */
    update({id, data}) {
      return apiHandler.put(`kyc-documents/${id}`, data);
    },
    accept({id}) {
      return apiHandler.post(`kyc-documents/${id}/acceptance`);
    },
    matches({id, data}) {
      return apiHandler.post(`kyc-documents/${id}/matches`, data);
    },
    reject({id, data}) {
      return apiHandler.post(`kyc-documents/${id}/rejection`, data);
    },
    review({id}) {
      return apiHandler.post(`kyc-documents/${id}/review`);
    },
  };
}
